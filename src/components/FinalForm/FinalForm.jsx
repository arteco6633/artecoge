...
const FinalForm = () => {
  ...
  useEffect(() => {
    fetch('https://ipapi.co/json/')
      .then(res => res.json())
      .then(data => { if (data.country_code) setCountry(data.country_code.toLowerCase()); })
      .catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        if (window.gtag) {
          window.gtag('event', 'generate_lead', { event_category: 'form', event_label: 'footer_form' });
        }
        setIsSubmitted(true);
          ...
          <div className="ff-content-grid">
            ...
            <div className="ff-info">
              ...
              <div className="ff-socials-row">
                <span className="ff-social-label">{f.socialLabel}</span>
            ...
            <div className="ff-form-box">
              {isSubmitted ? (
                <div className="ff-success-screen">
                  ...
                  <h3 className="ff-form-title">{f.successTitle}</h3>
                  <p className="ff-form-subtitle">{f.successText}</p>
                  <button className="ff-submit-btn" onClick={() => setIsSubmitted(false)}>{f.successAgain}</button>
                </div>
              ) : (
                <>
